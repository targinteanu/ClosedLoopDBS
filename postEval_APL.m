...
%% AR model for artifact removal 

% detect artifacts 
artExtend = 10; % extend artifact by __ samples 
artIndAll = StimTrainRec1; 
...
dataBaseline = dataOneChannel1(baselineStartInd:baselineEndInd); 
dataBaseline = Myeegfilt(dataBaseline,SamplingFreqAPL,13,30);
...
ARmdl = ar(iddata(dataBaseline', [], 1/SamplingFreqAPL), ARlen, 'yw');
end

%% normalize & mean-correct 
...
for ind = artIndAll
    ind0 = ind - ARlen;
    if (ind0 > 0) && (ind <= length(dataOneChannel1))
        dataOneChannel1(ind) = myFastForecastAR(ARmdl, dataOneChannel1(ind0:(ind-1))', 1);
    end
end

% plot artifact removal 
dataOneChannelWithArtifact1 = dataOneChannelWithArtifact - mean(dataOneChannelWithArtifact);
dataOneChannelWithArtifact1 = dataOneChannelWithArtifact1 / std(dataOneChannelWithArtifact1);
figure; 
ax(1) = subplot(211); 
plot(t, dataOneChannelWithArtifact1, 'k'); 
grid on; hold on; 
plot(t1, dataOneChannel1, 'b'); 
title('Artifact Removal'); ylabel(channelName);
ax(2) = subplot(212); 
plot(t, StimTrainRec, 'k');
grid on; hold on; 
plot(t1, StimTrainRec1, 'b'); 
ylabel(channelNameStim);
